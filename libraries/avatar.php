...
class CAvatar {
	...
	public function CAvatar( $path = null , $forceCreate = false)
	{
		...
		if($forceCreate)
		{
			if(!JFile::exists($this->_path))
			{
				...
				JFile::copy( JPATH_ROOT .'/components/com_community/index.html' , $this->_path .'/index.html' );
			}
		}
	}
	...
	public function draw( $filename , $image , $width , $thumbnail = false, $maxWidth = false)
	{
		...
		JFile::upload( $image['tmp_name'] , $this->_path .'/'. $filename );
		...
		if( $maxWidth == false || (($maxWidth != false) && ($currentWidth > $maxWidth)) )
		{
			...
			if(JFile::write( $this->_path .'/'. $filename , $content ))
			{
		...
		if($thumbnail)
		{
			...
			if(JFile::write( $this->_path .'/'. $thumbFile , $content ))
			{
