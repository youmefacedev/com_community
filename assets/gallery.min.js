...
},urlPhotoId:function(a){if(a==undefined){return jsPlaylist.customSetting.defaultId;}else{if(window.history.pushState){window.history.pushState("string","title",jsPlaylist.photos[joms.gallery.getPlaylistIndex(a)].sefURL);
...
joms.gallery.displayPhoto(joms.gallery.currentPhoto());joms.gallery.editablePhotoCaption();joms.gallery.swipeablePhoto();},displayViewport:function(){var g=joms.jQuery("#cGallery .photoViewport");
var h=(joms.jQuery(window).width()<=640)?true:false;g.unbind();g.hover(function(){joms.jQuery("#cGallery .photoAction").fadeIn("fast");},function(){if(!h){joms.jQuery("#cGallery .photoAction").fadeOut("fast");
}});var b=joms.jQuery("#cGallery .photoDisplay");b.css("height",Math.floor(b.width()/16*12));var a=joms.jQuery("#cGallery .photoLoad");a.css({top:Math.floor((b.height()/2)-(a.height()/2)),left:Math.floor((b.width()/2)-(a.width()/2))});
var c,j={};var i=joms.jQuery("#cGallery .photoActions");var f=joms.jQuery("#cGallery .photoAction._next");var e=joms.jQuery("#cGallery .photoAction._prev");
if(!h){j={top:Math.floor((b.height()/2)-(f.height()/2)),right:0};c={top:Math.floor((b.height()/2)-(e.height()/2)),left:0};}else{j={top:Math.floor((b.height()/2)-(f.height()/2)),right:0,"margin-right":0,opacity:0.3};
c={top:Math.floor((b.height()/2)-(e.height()/2)),left:0,"margin-left":0,opacity:0.3};}i.css({width:b.width(),height:0,top:0,left:0});f.css(j);e.css(c);
var d=joms.jQuery("#cGallery .photoTags");d.css({width:b.width(),height:0,top:0,left:0});},displayPhoto:function(a){var b=joms.jQuery("#cGallery .photoLoad");
clearTimeout(joms.gallery.loaderTimeout);joms.gallery.loaderTimeout=setTimeout("joms.jQuery('#cGallery .photoLoad').show()",300);joms.gallery.currentPhoto(a);
joms.jQuery("#cGallery .photoAction._next img").attr("src",joms.gallery.nextPhoto().thumbnail);joms.jQuery("#cGallery .photoAction._prev img").attr("src",joms.gallery.prevPhoto().thumbnail);
joms.gallery.displayPhotoCaption(a.caption);joms.gallery.createPhotoImage(a,function(c){var f=joms.jQuery("#cGallery .photoDisplay");var e=joms.jQuery("#cGallery .photoImage");
f.empty();c.appendTo(f);c.css({top:"3000px",left:"4000px",visibility:"visible",position:"absolute"});var d=c[0].height;var h=c[0].width;if(c[0].height>f.height()){h=h*(f.height()/d);
d=f.height();}if(h>f.width()){d=d*(f.width()/h);h=f.width();}if(d==0){h=joms.jQuery(".photoDisplay img").width();d=joms.jQuery(".photoDisplay img").height();
}var g={width:h,height:d,top:Math.floor((f.height()-d)/2),left:Math.floor((f.width()-h)/2),visibility:"visible",display:"none"};c.data("properties",g).css(g);
joms.gallery.switchPhoto(a.id);joms.gallery.displayPhotoTags(a.tags);});joms.gallery.prefetchPhoto([joms.gallery.prevPhoto(),joms.gallery.nextPhoto()]);
...
joms.gallery.createPhotoTag(c);joms.jQuery(".photoTextTags").empty();joms.gallery.createPhotoTextTag(c);},addPhotoTag:function(c){var b=joms.jQuery("#cGallery .photoTags");
var a=b.data("newPhotoTag");if(c>0){jax.call("community","photos,ajaxAddPhotoTag",a.photoId,c,a.top,a.left,a.width,a.height);}joms.gallery.cancelNewPhotoTag();
},removePhotoTag:function(a){jax.call("community","photos,ajaxRemovePhotoTag",a.photoId,a.userId);joms.gallery.clearPhotoTag(a);joms.gallery.clearPhotoTextTag(a);
var b=jsPlaylist.photos[joms.gallery.getPlaylistIndex(a.photoId)].tags;joms.jQuery.each(b,function(c){if(this.id==a.id){b.splice(c,1);}});},clearPhotoTag:function(a){if(a==undefined){joms.jQuery("#cGallery .photoTag").remove();
}else{joms.jQuery("#photoTag-"+a.id).remove();}},showPhotoTag:function(b,a){joms.jQuery("#photoTag-"+b).addClass("show"+a);},hidePhotoTag:function(a){joms.jQuery("#photoTag-"+a).removeClass("show showLabel showForce");
},createPhotoTextTag:function(tags){var photoTextTags=joms.jQuery(".photoTextTags");if(tags.length==0){photoTextTags.html("");}if(typeof(tags)=="string"){tags=eval("("+tags+")");
}var singleTag=false;if(!joms.jQuery.isArray(tags)){tags=[tags];singleTag=true;}var newPhotoTextTags=new Array();var a=1;if(joms.jQuery(".photoTextTags").children().length>0){if(joms.jQuery(".photoTextTags .photoTextTag .photoTextTagActions").length>0){joms.jQuery(".photoTextTags .photoTextTag .photoTextTagActions").append(", ");
}else{joms.jQuery(".photoTextTags .photoTextTag").append(", ");}}joms.jQuery.each(tags,function(i,tag){if(tag.id==undefined){return;}var photoTextTag=joms.jQuery('<span class="photoTextTag"></span>');
photoTextTag.data("tag",tag).attr("id","photoTextTag-"+tag.id).hover(function(){joms.gallery.showPhotoTag(tag.id,"Force");},function(){joms.gallery.hidePhotoTag(tag.id);
}).appendTo(photoTextTags);var photoTextTagLink=joms.jQuery("<a>");photoTextTagLink.attr("href",tag.profileUrl).html(tag.displayName).prependTo(photoTextTag);
...
}else{joms.jQuery("#photoTextTag-"+a.id).remove();joms.gallery.commifyTextTags();}},startTagMode:function(){joms.jQuery(".photoTags").unbindSwipe();joms.jQuery("#cGallery .photoTagInstructions").slideDown("fast");
...
a.hide();joms.jQuery("#cGallery .photoTagInstructions").hide();joms.jQuery("#startTagMode").show();joms.jQuery("#photoTagQuery").val("");joms.gallery.swipeablePhoto();
cWindowHide();},selectNewPhotoTagFriend:function(){var a=joms.jQuery("#cGallery .photoTagFriend");cWindowShow(function(){joms.gallery.showPhotoTagFriends();
},jsPlaylist.language.COM_COMMUNITY_SELECT_PERSON,300,300);cWindowActions('<button class="button" onclick="joms.gallery.confirmPhotoTagFriend();">'+jsPlaylist.language.COM_COMMUNITY_CONFIRM+"</button>");
...
var a=c.data("newPhotoTag",{});joms.jQuery("#photoTagQuery").val("");joms.jQuery("div.autocomplete-w1").parent().hide();joms.gallery.stopTagMode();joms.jQuery("#cGallery .photoTag").addClass("showLabel");
},displayCreator:function(a){jax.call("community","photos,ajaxDisplayCreator",a);},setProfilePicture:function(){var a="jax.call('community', 'photos,ajaxLinkToProfile', '"+joms.gallery.currentPhoto().id+"');";
cWindowShow(a,"",450,100);},rotatePhoto:function(b){if(joms.jQuery("#startTagMode").css("display")=="none"){return false;}var a=joms.gallery.currentPhoto().id;
joms.ajax.call("photos,ajaxRotatePhoto",[a,b],{success:function(c,e,f){var d=jsPlaylist.photos[joms.gallery.getPlaylistIndex(c)];d.url=e;d.thumbnail=f;
joms.gallery.displayPhoto(d);}});},swipeablePhoto:function(){joms.jQuery(".photoTags").unbindSwipe().touchSwipe(function(a){clearTimeout(joms.gallery.loaderTimeout);
if(a=="left"){joms.gallery.displayPhoto(joms.gallery.nextPhoto());}else{joms.gallery.displayPhoto(joms.gallery.prevPhoto());}joms.photos.photoSlider.switchPhoto();
});}}});function getPlaylistIndex(a){joms.gallery.getPlaylistIndex(a);}function nextPhoto(a){joms.gallery.nextPhoto(a);}function prevPhoto(a){joms.gallery.prevPhoto(a);
}function currentPhoto(a){joms.gallery.currentPhoto(a);}function urlPhotoId(a){joms.gallery.urlPhotoId(a);}function initGallery(){joms.gallery.init();}function displayViewport(){joms.gallery.displayViewPort();
}function displayPhoto(a){joms.gallery.displayPhoto(a);}function createPhotoImage(a,b){joms.gallery.createPhotoImage(a,b);}function prefetchPhoto(a){joms.gallery.prefetchPhoto(a);
}function getPhotoUrl(a){joms.gallery.getPhotoUrl(a);}function displayPhotoCaption(a){joms.gallery.displayPhotoCaption(a);}function editPhotoCaption(){joms.gallery.editPhotoCaption();
}function cancelPhotoCaption(){joms.gallery.cancelPhotoCaption();}function savePhotoCaption(){joms.gallery.savePhotoCaption();}function updatePhotoCaption(a,b){joms.gallery.updatePhotoCaption(a,b);
}function displayPhotoWalls(a){joms.gallery.displayPhotoWalls(a);}function setPhotoAsDefault(){joms.gallery.setPhotoAsDefault();}function removePhoto(){joms.gallery.confirmRemovePhoto();
}function downloadPhoto(){joms.gallery.downloadPhoto();}function updatePhotoReport(a){joms.gallery.updatePhotoReport(a);}function newPhotoTag(a){joms.gallery.newPhotoTag(a);
}function createPhotoTag(a){joms.gallery.createPhotoTag(a);}function drawPhotoTag(a,b){joms.gallery.drawPhotoTag(a,b);}function updatePlaylistTag(a){joms.gallery.updatePlaylistTag(a);
}function displayPhotoTags(a){joms.gallery.displayPhotoTags(a);}function addPhotoTag(a){joms.gallery.addPhotoTag(a);}function removePhotoTag(a){joms.gallery.removePhotoTag(a);
}function clearPhotoTag(a){joms.gallery.clearPhotoTag(a);}function showPhotoTag(b,a){joms.gallery.showPhotoTag(b,a);}function hidePhotoTag(a){joms.gallery.hidePhotoTag(a);
}function createPhotoTextTag(a){joms.gallery.createPhotoTextTag(a);}function commifyTextTags(){joms.gallery.commifyTextTags();}function clearPhotoTextTag(a){joms.gallery.clearPhotoTextTag(a);
}function startTagMode(){joms.gallery.startTagMode();}function stopTagMode(){joms.gallery.stopTagMode();}function selectNewPhotoTagFriend(){joms.gallery.selectNewPhotoTagFriend();
}function confirmPhotoTagFriend(){joms.gallery.confirmPhotoTagFriend();}function showPhotoTagFriends(){joms.gallery.showPhotoTagFriends();}function filterPhotoTagFriend(a){joms.gallery.filterPhotoTagFriend(a);
}function cancelNewPhotoTag(){joms.gallery.cancelNewPhotoTag();}function displayCreator(a){joms.gallery.displayCreator(a);}function setProfilePicture(){joms.gallery.setProfilePicture();
}