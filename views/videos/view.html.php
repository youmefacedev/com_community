...
class CommunityViewVideos extends CommunityView
{
	...
	private function _getVideosHTML( $videos, $pagination = NULL)
	{
		...
		$task			= $jinput->get->get('task' , '', 'WORD'); //JRequest::getVar( 'task' , '' , 'GET' );
		...
		$tmpl	= new CTemplate();
		return $tmpl->set( 'sort'				, $jinput->get('sort', 'latest','STRING') )  //JRequest::getVar('sort', 'latest') )
	...
	public function display($id= null)
	{
		...
		$featVideos	=   '';
		$featuredHTML	=   '';
		$sorted 	=   $jinput->get('sort', 'latest','STRING'); //JRequest::getVar('sort', 'latest');
	...
	**/
	...
	public function mypendingvideos($id= null)
	{
		...
		// Group video pending
		$groupid = $jinput->get('groupid', '0','INT'); //JRequest::getVar('groupid', '0');
		if(!empty($groupid))
		{
			$filters		= array
			(
	...
	public function _addSubmenu()
	{
		...
		$mainframe	= JFactory::getApplication();
		$jinput 	= $mainframe->input;
		$task		=   $jinput->request->get('task' , '', 'WORD'); //JRequest::getVar( 'task' , '' , 'REQUEST' );
		$groupId	=   $jinput->get->get('groupid' , '', 'INT'); //JRequest::getVar('groupid' , '' , 'GET');
